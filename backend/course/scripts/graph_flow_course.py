...
def do_graph(course):
    ...
    g = Digraph(
        "G",
        filename="../../graph/cursos/" + course.name + ".gv",
        node_attr={
            "shape": "record",
            "width": "3",
            "height": "1.2",
    ...
    for semester in range(1, course.num_semester + 1):
        with g.subgraph(name=("Periodo " + str(semester))) as subgraph_periodo:
            ...
            semester_dict = list(
                filter(lambda subject: subject["semester"] == semester, flow)
            )
            ...
            for subject in semester_dict["subjects"]:
                ...
                node_info = (
                    "{"
                    + f' <codigo>{str(subject["code"])} | <nome> {subject["subject_name"]} |'
                    + "{"
                    + f'{subject["status"]} | {subject["credit"]}'
                    + "}"
                    + "}"
                )
                subgraph_periodo.node(
                    str(subject["code"]),
                    node_info,
                    fillcolor=get_color(subject["pass_percent"]),
                )
